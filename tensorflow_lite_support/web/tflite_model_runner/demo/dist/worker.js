  ...
  if (message) {
    ...
    switch (message.data[0]) {
      case 'create':
        ...
        // Set webnn-polyfill backend if needed with sync API
        if (navigator.isElectron) {
          if (navigator.ml.createContext().tf !== undefined) {
            const tf = navigator.ml.createContext().tf;
            const devicePreference = parseInt(message.data[3]);
        ...
        } else if (navigator.ml.createContextSync().tf !== undefined) {
          const tf = navigator.ml.createContextSync().tf;
          const devicePreference = parseInt(message.data[3]);
          if (devicePreference === 1) {
            tf.setBackend('webgl');
          } else {
